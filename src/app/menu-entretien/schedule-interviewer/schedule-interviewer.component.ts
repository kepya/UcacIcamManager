...
import {
  CompteDisponibilite,
  Disponibility,
} from 'src/app/shared/models/entretient';
...
export class ScheduleInterviewerComponent implements OnInit {
  ...
  selectCandidatMap: Map<string, { label: string; candidat: Candidature }> =
    new Map<string, any>();
    ...
    items: [
      {
        ...
        candidat: {
          ...
          compte: {
            ...
            prenom: 'danielle',
            password:
              '$2a$10$iyL5gzCHSQtqTRhUu.k8Q.yZ7e.W3sJRrs7ZNwB278BLRadnZ2e3q',
  ...
  constructor(
    private disponibilitySrv: DisponibiliteService,
    private candidatureSrv: CandidatureService,
    private sessionSrv: SessionExamenService,
    private zoneService: ZoneService,
    private compteSrv: CompteService,
    public commonService: CommonService,
    private noteService: NoteService,
    private compteDisponibiliteService: CompteDisponibiliteService,
  ...
  getCandidatures(idZone: number) {
    this.candidatureSrv.nonEntretenuByZone(idZone).subscribe({
      next: (candidatures) => {
        candidatures = candidatures.filter(
          (c) => c.solvable == true && c.statut == 'Admissible'
        );
        ...
        let faCandidats = candidatures
          .filter((v) => v.formation1 === 'FA')
          .map((r) => ({
        ...
        let ipCandidats = candidatures
          .filter((v) => v.formation1 === 'IP')
          .map((r) => ({
            label: r?.compte?.name + ' ' + r?.compte?.prenom,
            candidat: r,
          }));
        ...
        let iCandidats = candidatures
          .filter((v) => v.formation1 === 'I')
          .map((r) => ({
            label: r?.compte?.name + ' ' + r?.compte?.prenom,
            candidat: r,
          }));
        ...
        let xCandidats = candidatures
          .filter((v) => v.formation1 === 'X')
          .map((r) => ({
            label: r?.compte?.name + ' ' + r?.compte?.prenom,
            candidat: r,
          }));
        ...
        let lCandidats = candidatures
          .filter((v) => v.formation1 === 'L')
          .map((r) => ({
            label: r?.compte?.name + ' ' + r?.compte?.prenom,
            candidat: r,
          }));
        ...
        let opCandidats = candidatures
          .filter((v) => v.formation1 === 'OP')
          .map((r) => ({
            label: r?.compte?.name + ' ' + r?.compte?.prenom,
            candidat: r,
          }));
  ...
  ngOnInit(): void {
    ...
    this.getCandidatures(this.selectedZone.id || 1);
    this.getZones();
  ...
  handleInterviewerSelect(event: any, rowIndex: number) {
    ...
    let result = this.alreadyUseJuries.find(
      (j) => j.id == parseInt(jury.value)
    );
    if (result) {
      this.messageService.add({
        ...
        detail:
          'Ce jury a déjà été utilisé pour cette plage veuillez changer de jury',
      });
    } else {
      let key =
  ...
  handleFormationSelect(event: any): void {
    ...
    this.filterGroupedFormations = this.groupedFormations.filter(
      (f) => f.label == formation.code
    );
  }
  ...
  handleCandidatSelect(event: any, horaire: string, rowIndex: number) {
    let candidat = event.value as unknown as {
      label: string;
      candidat: Candidature;
    };

    let result = this.alreadyUseCandidats.find(
      (c) => c.id == candidat.candidat.id
    );
    ...
    if (result) {
      this.messageService.add({
        ...
        summary: `Erreur d'assignation`,
        detail:
          'Ce candidat a déjà été utilisé pour cette plage veuillez changer de candidat',
      });
    } else {
      let key =
        this.currentDay.getDate() +
      ...
      if (this.noteRequestMaps.has(key)) {
        ...
        this.selectCandidatMap.set(key, {
          ...
          label:
            candidat.candidat!.compte.name +
            ' ' +
            candidat.candidat!.compte.prenom,
        });
      } else {
        let dates = this.commonService.buildDate(this.currentDay, horaire);
        this.noteRequestMaps.set(key, {
  ...
  getCandidat(noteRequest?: NoteRequest) {
    let candidat = this.candidats.find(
      (c) => c.id == noteRequest?.candidatureid
    );
    if (candidat) {
      return {
  ...
  randomizeCandidatDisplay() {
    ...
    if (
      (this.alreadyUseCandidats.length == Juryslength &&
        this.alreadyUseJuries.length == Juryslength) ||
      this.numberOfCandidatToInterview == 1
    ) {
    ...
    this.disponibilitySrv.liste().subscribe({
      next: (disponibilities: Disponibility[]) => {
        ...
        for (let index = 0; index < disponibilities.length; index++) {
          ...
          let startTime = this.commonService.formatDate(
            disponibilite!.debut_disponibilite
          );
          let endTime = this.commonService.formatDate(
            disponibilite!.fin_disponibilite
          );
    ...
    this.compteDisponibiliteService.liste().subscribe({
      next: (disponibilities: CompteDisponibilite[]) => {
        for (let index = 0; index < disponibilities.length; index++) {
          ...
          let disponibility = this.disponibilities.find(
            (d) => d.id == element.disponibilite!.id
          );

          let startTime = this.commonService.formatDate(
            disponibility!.debut_disponibilite
          );
          let endTime = this.commonService.formatDate(
            disponibility!.fin_disponibilite
          );
          let horaire = startTime + ' - ' + endTime;
  ...
  validate() {
      ...
      if (noteRequest?.candidatureid && noteRequest?.candidatureid > 0) {
        this.noteService.create(noteRequest).subscribe({
          next: (res) => {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Assignation du jury pour entretien',
              detail:
                'Assignation du jury pour un entretien avec le candidat effectuée avec success pour la date du ' +
    ...
    this.getCandidatures(this.selectedZone.id || 0);
    setTimeout(() => {
